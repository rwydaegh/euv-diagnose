...
function phase(): string {
  return `<section class="phase-summary"><h2>Phase difference <span class="phase-value">${number(data.synthetic.phaseDifferenceDeg)}°</span></h2><p>At 13.5 nm and 6°.</p><p class="fine">Relative phase is arg(r<sub>absorber</sub> / r<sub>multilayer</sub>) at a common reference plane.</p></section>`;
}
...
function syntheticView(): string {
  ...
  return `<div class="results-layout"><section class="card spectra-card"><h2>Two synthetic stacks</h2><p class="card-intro">The model predicts nearly overlapping intensity spectra for these stacks, with different reflection phases.</p><div class="channel-switch" role="group" aria-label="Reflectivity channel"><button data-channel="1" aria-pressed="${channel === 1}">Multilayer region</button><button data-channel="0" aria-pressed="${channel === 0}">Absorber region</button></div>${plot(
    ...
    "Reflectivity (1)",
  )}<div class="difference-header"><h3>Intensity difference / assumed noise</h3></div>${plot(s.wavelengths, [{ ys: diff, color: "#626b76", label: "(B − A) / σ" }], "Difference between candidate intensities divided by illustrative noise", "Difference / σ (1)")}</section><aside class="side-column">${phase()}<section class="card measurement">${angleControls(s.angles)}<div class="distance"><div><span>Pairwise spectral distance</span><strong>${number(s.distances[ai])}</strong></div><small>Combined absorber + multilayer, at ${angle}°. Euclidean norm of (B − A)/σ over both channels and all wavelengths.</small></div><button class="primary" id="reveal">${angle === 30 ? "Return to the 6° view" : "Compare at 30°"} <span aria-hidden="true">→</span></button><p class="fine">The noise assumption is the same at every angle. Comparing these two candidates does not establish a unique solution.</p></section></aside></div><p class="model-summary">Both stacks use the same TaN absorber / Mo–Si reflector model with 40 multilayer periods. The model calculates intensity and phase from reflections at the interfaces.</p>`;
}
function measuredView(): string {
  ...
  return `<div class="results-layout"><section class="card spectra-card"><h2>Measured reflectivity and refit</h2><p class="card-intro">${escape(m.metadata.description)}</p><div class="channel-switch" role="group" aria-label="Reflectivity channel"><button data-channel="1" aria-pressed="${channel === 1}">Multilayer region</button><button data-channel="0" aria-pressed="${channel === 0}">Absorber region</button></div>${plot(
    m.wavelengths,
    [
...
function learningView(): string {
  ...
  return `<section class="card benchmark"><h2>${escape(l.title)}</h2><p class="card-intro">${escape(l.description)}</p><div class="table-scroll"><table class="metrics-table"><caption class="sr-only">Benchmark results</caption><thead><tr><th scope="col">Metric</th><th scope="col">Result</th><th scope="col">Evaluation</th></tr></thead><tbody>${l.metrics.map((m) => `<tr><th scope="row">${escape(m.label)}</th><td>${escape(m.value)}</td><td>${m.detail ? escape(m.detail) : ""}</td></tr>`).join("")}</tbody></table></div>${scatter()}<p class="benchmark-note">The estimator was trained on simulations. Its accuracy on experimental data is unknown. Evaluation assumptions and limitations are listed below.</p></section>`;
}
function render(): void {
  root.innerHTML = `<a class="skip-link" href="#workspace">Skip to results</a><header><a class="project-name" href="./" aria-label="EUV Diagnose home">euv-diagnose</a><a href="https://github.com/s-sherwin/EUV" target="_blank" rel="noopener noreferrer" aria-label="Original EUV data and code">Source data ${arrow}</a></header><main><section class="page-intro"><h1>EUV reflectivity</h1><p>Multilayer calculations, measured refits and phase estimation from simulated spectra.</p><p class="fine">Wavelength range: 12.5–14.5 nm. Reflectivity is an intensity fraction. Angles are measured from the surface normal.</p></section><div class="workspace-toolbar" id="workspace"><nav class="tabs" aria-label="Experiment"><button data-tab="synthetic" aria-current="${tab === "synthetic" ? "page" : "false"}">Phase ambiguity</button><button data-tab="measured" aria-current="${tab === "measured" ? "page" : "false"}">Measured spectra</button><button data-tab="learning" aria-current="${tab === "learning" ? "page" : "false"}">Learned inference</button></nav></div><div id="experiment">${tab === "synthetic" ? syntheticView() : tab === "measured" ? measuredView() : learningView()}</div><section class="provenance"><div class="data-tools"><span>Download</span><div class="export-buttons"><button id="export-json">Result JSON ↓</button><button id="export-csv" ${tab === "learning" ? "disabled" : ""}>Spectra CSV ↓</button><button id="export-svg" ${tab === "learning" ? "disabled" : ""}>Plot SVG ↓</button></div></div><button id="provenance-toggle" aria-expanded="${expanded}" aria-controls="provenance-content"><span>Methods and assumptions</span><span aria-hidden="true">${expanded ? "−" : "+"}</span></button><div id="provenance-content" ${expanded ? "" : "hidden"}><div class="provenance-grid"><div><ul>${(tab === "synthetic" ? data.synthetic.metadata.assumptions : tab === "measured" ? (data.measured?.metadata.limitations ?? []) : (data.learning?.limitations ?? [])).map((s) => `<li>${escape(s)}</li>`).join("")}</ul></div><div><p>The downloads contain saved calculations. The browser does not fit a model.</p><a href="${safeLink(tab === "measured" ? (data.measured?.metadata.sourceUrl ?? "") : data.synthetic.metadata.sourceUrl)}" target="_blank" rel="noopener noreferrer">Original data and model ${arrow}</a><p class="fine">${data.revision ? `Code revision: ${escape(data.revision)}` : ""}${data.generatedAt ? `<br>Generated: ${escape(data.generatedAt)}` : ""}</p></div></div></div></section></main><footer><span>Saved research results · No instrument connection</span></footer><div id="announcement" class="sr-only" role="status" aria-live="polite"></div>`;
  bind();
}
